...
public class APIController {
    ...
    public String attraction_api(Model model, HttpServletRequest request, HttpSession session) {
    	...
    	if(select1 != null && !select1.equals("")) {
    		String user_select1 = apiservice.attractionAPI("http://localhost:5000/api/attraction", "attraction", select1, "500", "json", "20231121");
    		String[] sb_select1 = user_select1.split("\"");
        	
    		model.addAttribute("select1",namelist.get(Integer.parseInt(select1) - 1));
        	model.addAttribute("sb_select1", namelist.get(Integer.parseInt(sb_select1[1]) - 1));
    	}
    	if(select2 != null && !select2.equals("")) {
    		String user_select2 = apiservice.attractionAPI("http://localhost:5000/api/attraction", "attraction", select2, "500", "json", "20231121");
    		String[] sb_select2 = user_select2.split("\"");
        	
    		model.addAttribute("select2",namelist.get(Integer.parseInt(select2) - 1));
        	model.addAttribute("sb_select2", namelist.get(Integer.parseInt(sb_select2[1]) - 1));
    	}
    	if(select3 != null && !select3.equals("")) {
    		String user_select3 = apiservice.attractionAPI("http://localhost:5000/api/attraction", "attraction", select3, "500", "json", "20231121");
    		String[] sb_select3 = user_select3.split("\"");
        	
    		model.addAttribute("select3",namelist.get(Integer.parseInt(select3) - 1));
        	model.addAttribute("sb_select3", namelist.get(Integer.parseInt(sb_select3[1]) - 1));
    	}
